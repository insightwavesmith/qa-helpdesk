-- =============================================================
-- HOTFIX: purchase + omni_purchase 중복집계 수정
-- 근본 원인: fn_extract_daily_metrics 트리거가 SUM으로 양쪽을 합산
-- 수정: omni_purchase 우선, purchase 폴백 (TypeScript calculateMetrics와 동일)
-- =============================================================
...
BEGIN
  ...
  -- ★ HOTFIX: omni_purchase 우선, purchase 폴백 (SUM 제거)
  SELECT COALESCE(
    (SELECT (a->>'value')::integer FROM jsonb_array_elements(_actions) a WHERE a->>'action_type' = 'omni_purchase' LIMIT 1),
    (SELECT (a->>'value')::integer FROM jsonb_array_elements(_actions) a WHERE a->>'action_type' = 'purchase' LIMIT 1),
    0
  ) INTO _purchases;

  SELECT COALESCE(
    (SELECT (a->>'value')::numeric FROM jsonb_array_elements(_action_values) a WHERE a->>'action_type' = 'omni_purchase' LIMIT 1),
    (SELECT (a->>'value')::numeric FROM jsonb_array_elements(_action_values) a WHERE a->>'action_type' = 'purchase' LIMIT 1),
    0
  ) INTO _purchase_value;

  -- ★ HOTFIX: initiate_checkout도 동일 패턴
  SELECT COALESCE(
    (SELECT (a->>'value')::integer FROM jsonb_array_elements(_actions) a WHERE a->>'action_type' = 'initiate_checkout' LIMIT 1),
    (SELECT (a->>'value')::integer FROM jsonb_array_elements(_actions) a WHERE a->>'action_type' = 'omni_initiated_checkout' LIMIT 1),
    0
  ) INTO _initiate_checkout;
  ...
  -- 참여 지표 (post_reaction 우선, like 폴백)
  SELECT COALESCE(
    (SELECT (a->>'value')::integer FROM jsonb_array_elements(_actions) a WHERE a->>'action_type' = 'post_reaction' LIMIT 1),
    (SELECT (a->>'value')::integer FROM jsonb_array_elements(_actions) a WHERE a->>'action_type' = 'like' LIMIT 1),
    0
  ) INTO _reactions;
...
-- =============================================================
-- 기존 데이터 보정: raw_insight가 있는 모든 행에 트리거 재실행
-- UPDATE ... SET raw_insight = raw_insight → BEFORE UPDATE 트리거 발동
-- =============================================================
UPDATE daily_ad_insights
SET raw_insight = raw_insight
