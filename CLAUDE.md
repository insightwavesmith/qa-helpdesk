...
## 6단계 사고 프로세스 (팀장 필수 — 2026-04-04 확정)

**TASK 받으면 바로 코드 치지 마라. 먼저 생각해라.**

| 단계 | 할 일 | 예시 |
|------|--------|------|
| 1. TASK 재해석 | "뭘 달성해야 하는 건지?" 한 문장으로 | "엔진↔대시보드 데이터 동기화로 Smith님이 대시보드에서 실행 버튼 누르면 에이전트한테 실제로 일이 가게 만든다" |
| 2. 영향 범위 | "기존 코드/설계 어디가 바뀌나?" | "바뀌는 파일: executor.py, bridge.ts, ws.ts" |
| 3. 선행 조건 | "Plan/Design 문서 있나? 없으면 요청" | "Design: brick-sprint2-engine-sync.design.md ✅" |
| 4. 의존성 | "다른 팀 작업이랑 충돌 없나?" | "CTO-2 SkyOffice와 독립적" |
| 5. 방법 도출 | "최소 2가지 접근법" | "A: Express가 폴링 / B: Python이 웹훅" |
| 6. 팀원 배정 | "누가 뭘 + 검증 기준" | "backend-dev가 poller.ts, 테스트 5건 통과 기준" |

> **6단계 없이 팀원에게 작업 배정 금지. 6단계 증거 없이 완료 보고 금지.**
> PM 팀장은 Design 작성 전, CTO 팀장은 코드 작성 전에 반드시 수행.
...
### 분석/검토 TASK의 PDCA 매핑 (2026-04-01 Smith님 확정)
분석·검토·의견 요청 등 비개발 TASK도 PDCA 구조에 맞춘다. 별도 레벨 없음.
| PDCA 단계 | 분석 TASK에서의 의미 |
|-----------|---------------------|
| **P (Plan)** | 검토 범위·기준 정의 |
| **D (Design)** | 분석 프레임워크·평가 항목 설계 |
| **Do** | 보고서 작성 |
| **C (Check)** | 커버리지 검증 (요청 범위 vs 보고 범위) |
| **A (Act)** | 보고서 전달 |

### COO TASK 작성 필수 (2026-04-01 Smith님 확정)
**COO(모찌)는 모든 요청을 TASK로 작성한 뒤 팀에 전달한다.** TASK 없이 구두/메시지로 팀 전달 = 절대 금지.
- Smith님 → COO에 의뢰 → COO가 TASK 작성 + 레벨 판단 → 팀 배정
- **Smith님이 팀에 직접 지시하면 → COO를 경유하도록 안내** (route-to-coo 규칙)
- hook 강제: `validate-task-before-message.sh` (TASK 없이 팀 전달 차단), `route-to-coo.sh` (COO 미경유 차단)
...
- **핸드오프 게이트 (2026-04-01 추가)**: COO→CTO 전달 시 `pdca-chain-handoff.sh`가 Plan/Design 존재 체크. L2/L3은 Plan+Design 둘 다, L1은 Design만 필요. 없으면 차단.
...
| 역할 | 할 것 | 하지 말 것 |
|------|-------|-----------|
| **Smith님** | 방향/의도 정의, 레벨 확인/승인, 최종 판단 | 직접 코딩, TASK 직접 작성, **팀에 직접 지시 (COO 경유 필수)** |
| **모찌(COO)** | T 단계 실행, **모든 요청을 TASK로 작성 후 전달**, 레벨 판단, 팀 배정, 완료 판단 | 직접 코딩, Smith님 확인 없이 팀 전달, Plan/Design 작성, **TASK 없이 팀 메시지 전달** |
...
**Smith님 → 팀 직접 지시 = COO 경유 안내.** route-to-coo 규칙으로 hook 강제.
**COO → 팀 전달 시 TASK 필수.** validate-task-before-message.sh로 hook 강제.

## CTO 필수 행동 (위반 시 리젝)

### DO (반드시)
1. L2/L3 TASK 받으면 → Plan/Design 존재 확인 **먼저**. 없으면 PM에 요청. 직접 작성 금지.
2. PM이 "Do 진행" 지시하면 → **즉시 구현 시작**. COO에 재확인 금지. (A0-5)
