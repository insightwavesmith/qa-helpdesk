// __tests__/hooks/auto-shutdown.test.ts — auto-shutdown.sh 전체 흐름 테스트 (설계서 6-2)
// TDD Red: tmux/sleep 불가 → 레지스트리 JSON 상태 전이 + bash 함수 단위 테스트
//
// UT-1: 2명 모두 종료 → registry 모두 terminated
// INC-4: shutdown_pending → force-kill
// INC-5: shutdown_pending 후 미종료 → force_kill 전이
// E-1: tmux kill-pane 실패 → pane_dead
// E-4: pane_index=0 → [BLOCK] (Red — tmux 미사용)
// INC-6: PDCA updatedAt 자동 갱신
// INC-7: shutdownState === "done"
// E-2: registry 없으면 config에서 자동 생성
...
describe('auto-shutdown.sh (설계서 6-2)', () => {
  ...
  describe('tmux pane 처리', () => {
    ...
    it('E-4: pane_index=0 → kill 절대 안 함, [BLOCK] 출력 (Red — tmux 미사용)', () => {
      ...
      // Stage 2 로직 인라인 — tmux 미사용 환경에서 PANE_INDEX="" → [BLOCK] 안 나옴 (Red)
      const result = runBashFunction(funcsPath, [
        'member="backend-dev"',
      ...
      // tmux 없는 환경: PANE_INDEX="" → [NO-BLOCK] → 이 expect 실패 (Red)
