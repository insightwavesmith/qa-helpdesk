...
### 5. 배포 후 런타임 검증 (RET-004, 2026-03-26 추가)
- **배포 환경 체크리스트 필수** — IAM 역할, 환경변수, 네트워크 설정
- **배포 후 로그 확인 필수** — 실제 요청 1회 → 에러 로그 0건 확인
- **핵심 플로우 검증** — 로그인 → DB 쿼리 성공 → 데이터 표시 최소 1회
- **"gcloud run deploy 성공" ≠ "서비스 정상"** — 로그를 봐야 안다

### Smith님 액션 아이템
1. 마이그레이션 PR 분할 요구 습관화
2. "빌드 성공" ≠ "동작 정상" 인식
3. 쿼리빌더 vs ORM(Drizzle) 전략 결정 필요
4. "배포 성공" ≠ "서비스 정상" 인식 — 배포 후 로그인 1회 테스트 습관화
