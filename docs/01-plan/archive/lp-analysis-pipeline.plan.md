...
| 함수 | 입력 | 기대 출력 | 검증 포인트 |
|------|------|----------|------------|
| `analyzeLpScreenshot(lpId, screenshotUrl)` | `lpId: "lp_001"`, GCS signed URL | `{ reference_based: ReferenceBasedAnalysis }` (8개 카테고리 JSONB) | 8개 카테고리(page_structure~mobile_ux) 모두 non-null |
| `fetchMixpanelFunnel(accountId, lpUrl, period)` | `accountId: "act_123"`, `lpUrl: "https://shop.com/product/1"`, `period: "30d"` | `{ data_based: DataBasedAnalysis }` (퍼널 전환율 + 요소 교차분석) | view_page > 0, 각 단계 전환율 0~1 범위 |
| `computeAlignmentScore(creativeAnalysis, lpAnalysis)` | 소재 5축 결과 + LP reference_based | `{ message: number, cta: number, offer: number, visual: number, overall: number, issues: Issue[] }` | overall = 가중평균(message×0.35 + cta×0.25 + offer×0.25 + visual×0.15), 각 점수 0~100 |
| `computeReachToPurchaseRate(adInsights, lpFunnel)` | 광고 CTR + LP click_to_purchase_rate | `{ rate: number, ad_contribution: number, lp_contribution: number }` | rate = CTR × LP전환율, 값 0~1 범위 |
| `classifyBottleneck(adMetrics, lpMetrics)` | 광고 지표 + LP 지표 | `{ type: 'ad'|'lp'|'both', detail: string }` | CTR 높음+구매 낮음 → 'lp', CTR 낮음 → 'ad' |
| `parseMixpanelEvents(rawEvents, lpUrl)` | Mixpanel Export API 원시 데이터 | `{ view_page, view_product, add_to_cart, purchase, scroll_rate_avg, dwell_time_avg }` | 이벤트 카운트 ≥ 0, scroll_rate 0~100%, dwell_time ≥ 0 |

### T2. 엣지 케이스 정의

| # | 엣지 케이스 | 입력 조건 | 기대 동작 | 우선순위 |
|---|-----------|---------|---------|---------|
| E1 | LP 스크린샷 없음 | `lp_snapshots`에 해당 LP 없음 | 분석 건너뜀, `reference_based: null` 유지, 로그 경고 | P0 |
| E2 | Gemini Vision rate limit | 15 req/min 초과 | exponential backoff (4초→8초→16초), 최대 3회 재시도 | P0 |
| E3 | Mixpanel API 키 없음 | 수강생이 API 키 미제공 | `data_based: null` 유지, reference_based만으로 분석 진행 | P1 |
| E4 | LP 트래픽 0건 | Mixpanel `view_page` 0건 (신규 LP 또는 비활성) | 퍼널 전환율 전부 null, "데이터 수집 중" 표시 | P1 |
| E5 | creative_lp_map 매핑 없음 | 소재와 LP 연결 관계 미등록 | 자동 URL 매칭 시도 → 실패 시 일관성 분석 건너뜀 | P1 |
| E6 | LP 스크린샷 품질 불량 | 빈 화면, 로딩 중 캡처, CAPTCHA 페이지 | Gemini 분석 confidence 낮음 표시, 재크롤링 큐에 추가 | P1 |
| E7 | UTM→Meta 매핑 불완전 | utm_campaign이 Meta 캠페인 ID와 불일치 | 매핑 불가 트래픽 → "organic" 분류, reach_to_purchase_rate에서 제외 | P2 |
| E8 | 수강생 40명 = 소규모 벤치마크 | 카테고리별 LP N<10 | 전체 풀 벤치마크 사용, 카테고리별 세분화 보류 | P2 |
| E9 | 동적 LP (SPA) | 스크린샷이 초기 로딩 상태만 캡처 | Playwright waitForLoadState + 추가 스크롤 후 캡처, 3초 대기 | P1 |
| E10 | GCS signed URL 만료 | LP 스크린샷 URL 만료 (7일 기본) | URL 재생성 후 Gemini 호출, 실패 시 재크롤링 | P2 |

### T3. 모킹 데이터 (Fixture)

```json
// fixture: lp_analysis_reference_based — LP AI 분석 결과
{
  "page_structure": {
    "sections": ["hero", "benefits", "reviews", "pricing", "cta"],
    "page_length": "long",
    "estimated_scroll_depth": 4200,
    "first_cta_position": "above_fold"
  },
  "pricing_strategy": {
    "has_anchor_price": true,
  ...
  "trust_elements": {
    "certifications": ["정품인증"],
    "return_policy": "30일 무료 반품",
    "brand_story": true
  },
  "conversion_psychology": {
    "persuasion_triggers": ["social_proof", "scarcity"],
    "objection_handling": true,
    "benefit_priority": "price"
  },
  "mobile_ux": {
    "sticky_cta": true,
    "readability": "good",
...
```json
// fixture: lp_data_based — Mixpanel 행동 데이터 분석
{
  "funnel": {
    "view_page": 1250,
    "view_product": 890,
  ...
  "conversion_rates": {
    "page_to_product": 0.712,
    "product_to_cart": 0.175,
    "cart_to_purchase": 0.336,
    "click_to_purchase": 0.036
  },
  "behavior": {
    "avg_dwell_time_sec": 128,
    "avg_scroll_depth_percent": 68,
    "bounce_rate": 0.22,
    "return_visit_rate": 0.15
  },
  "period": "2026-02-25~2026-03-25",
  "total_sessions": 1250
}
```

```json
// fixture: creative_lp_alignment — 소재↔LP 일관성
{
  ...
  "cta_alignment": 85,
  "offer_alignment": 62,
  "visual_consistency": 71,
  "overall_score": 74.05,
  "issues": [
    {
      "type": "offer_mismatch",
      "severity": "high",
      "description": "소재에서 '50% 할인' 강조했으나 LP에서는 '30% 할인'으로 표시",
      "action": "LP 가격/할인 정보를 소재와 일치시키기"
    }
  ]
}
```
...
|-----------|---------------|----------------|
| LP 스크린샷 분석 (analyze-lps-v2) | `__tests__/lp-analysis/lp-screenshot-analysis.test.ts` | vitest |
| Mixpanel 퍼널 수집 | `__tests__/lp-analysis/mixpanel-funnel.test.ts` | vitest |
| 소재↔LP 일관성 점수 | `__tests__/lp-analysis/creative-lp-alignment.test.ts` | vitest |
| reach_to_purchase_rate 산출 | `__tests__/lp-analysis/reach-to-purchase-rate.test.ts` | vitest |
| 병목 분류 (ad vs lp) | `__tests__/lp-analysis/bottleneck-classifier.test.ts` | vitest |
| Mixpanel 이벤트 파서 | `__tests__/lp-analysis/mixpanel-event-parser.test.ts` | vitest |
| data_based JSONB 생성 | `__tests__/lp-analysis/data-based-builder.test.ts` | vitest |
| LP 대시보드 컴포넌트 | `__tests__/lp-analysis/lp-dashboard.test.tsx` | vitest + @testing-library/react |
