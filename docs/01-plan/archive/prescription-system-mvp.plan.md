...
> 작성일: 2026-03-25
> 작성자: PM팀
> 상태: Plan 완료 (2026-03-25 축 재설계 반영)
> 선행 조건: 5축 배치 완료 (현재 16% → 90%+ 필요)

---

## 0. 축 재설계 (2026-03-25 상관분석 기반 확정)

### 변경 배경

358개 소재 피어슨 상관분석 + 221개 임베딩 교차분석 결과:
- 저장 Lift 1.77x (구매 기여 최고), 댓글 1.01x (노이즈)
- 참여 지표 → 구매 직접 상관 r≈0 (간접 기여: 메타 알고리즘 참여율순위 경유)
- 참여축이 POOR일 때 액셔너블한 처방이 나오지 않음 ("더 공감가게" = 처방 아님)
- 임베딩 센트로이드 유사도 0.9541 → 시각적 유사성으로 성과 설명 불가, 메시지/카피가 결정

### 총가치각도기 2단 구조 (광고 + 랜딩 = 한 세트)

메타 총가치 3꼭지(추산행동률/광고품질/참여율순위)에 대응하되, 광고-LP를 분리:

```
[광고 각도기] — 소재가 컨트롤하는 범위
  P0 기반  : 3초시청률, ThruPlay, 지속비율     → 진단 + 처방 (광고품질)
  P1 참여  : 좋아요+댓글+공유+저장 합산        → 진단만 (참여율순위 모니터링)
  P2 전환  : CTR (광고의 끝점)                  → 진단 + 처방 (추산행동률)

[랜딩 각도기] — LP가 컨트롤하는 범위
  클릭 → 체류 → 장바구니 → 구매전환율          → 진단 + 처방

[세트 목적함수]
  reach_to_purchase_rate (노출→구매율) = 광고+랜딩 합쳐야 나오는 최종 성적표
```
...
| 항목 | 결정 | 근거 |
|------|------|------|
| 참여축 유지 여부 | **유지** (진단만) | 메타 알고리즘이 참여율순위를 독립적으로 평가 → 노출량/CPM에 영향 |
| 참여축 처방 여부 | **안 함** | "좋아요를 올려라" = 액셔너블하지 않음. 기반/전환 개선 시 참여도 따라옴 |
| 저장의 위치 | 참여 합산에 포함 + 전환 퍼널에서 별도 추적 | 하이브리드 지표 (Lift 1.77x) |
| 댓글의 위치 | 참여 합산에 포함, 진단 가중치 극소화 | Lift 1.01x = 노이즈 |
| CTR의 위치 | **광고 전환축의 끝점** (P2) | 소재가 만들 수 있는 마지막 행동 |
| 결제시작율/구매전환율 | **랜딩 각도기로 이동** | 클릭 이후는 LP 영역 |

### 진단 → 처방 흐름

```
광고 기반 POOR → 처방: "훅/비주얼/3초 시청률 개선" (구체적 5축 기반 액션)
광고 참여 POOR → 진단만: "메타 알고리즘 참여 점수 낮음" (처방 없음, 기반/전환 개선으로 해결)
광고 전환 POOR → 처방: "CTA/오퍼/메시지 개선" (CTR 올리기)
랜딩 POOR     → 처방: "LP 구조/메시지 일관성/전환 요소 개선"
```
...
| 참여축 POOR일 때 "광고가 눈에 안 띄어요" 한 줄이 전부 | 참여는 모니터링만, 기반/전환에 집중된 액셔너블한 처방 |
...
| 광고-LP가 별개로 진단됨 | 광고(~CTR) + 랜딩(CTR~구매) 세트로 병목 특정 |

### Smith님 비전과의 연결

> "메타 광고 = 광고 → 랜딩 → 구매까지 이어지는 **확률 게임**. 각 단계에서 이탈률을 줄이는 게 핵심." (SERVICE-VISION.md)

처방 시스템은 이 확률게임을 **광고 영역(~CTR)과 랜딩 영역(CTR~구매)**으로 나눠서, 병목이 어디인지 특정하고 각각에 맞는 처방을 제시한다. "문제 지적"에서 "해결 방법 제시"로의 진화.
...
| FR-07 | 참여축 모니터링 UI — 진단만 표시, 처방 비활성 | **P0** | "메타 참여율순위" 건강도 게이지 |
| FR-08 | 광고-LP 세트 점수 (reach_to_purchase_rate 병목 표시) | **P1** | CTR 높은데 구매 안 됨 → LP 문제 표시 |
| FR-09 | LP 일관성 분석 기반 LP 처방 (랜딩 각도기) | **P2** | V2 — Out of Scope |
| FR-10 | 처방 이력 저장 (before/after 비교용) | **P2** | 추후 확장 |
...
```json
{
  ...
  "ad_diagnosis": {
    "foundation": "기반 진단 (3초시청률/ThruPlay/지속비율 — 처방 대상)",
    "engagement": "참여 진단 (메타 알고리즘 건강도 — 모니터링만, 처방 없음)",
    "conversion": "전환 진단 (CTR — 광고의 끝점, 처방 대상)"
  },
  "set_bottleneck": {
    "diagnosis": "CTR 높은데 구매 안 됨 → LP 문제 / CTR 낮음 → 소재 문제",
    "reach_to_purchase_rate": "0.12%",
    "bottleneck": "ad | lp | both"
  },
  "weakness_analysis": [
    {
      "axis": "text",
...
처방은 **광고 영역(~CTR)**과 **랜딩 영역(CTR~구매)**으로 나뉜다:

#### 광고 각도기 (MVP 범위)

| 각도기 축 | 여정 단계 | 성과 지표 | 관련 5축 | 처방 가능 여부 | 처방 예시 |
|-----------|----------|----------|---------|:-----------:|----------|
| **P0 기반** | 감각 (보고+듣고) | 3초시청률, ThruPlay, 지속비율 | hook, visual, quality | ✅ 처방 | "첫 프레임 제품 클로즈업 → 3초시청률 개선" |
| **P1 참여** | — (알고리즘) | 좋아요+댓글+공유+저장 합산 | — | ❌ 진단만 | "메타 참여율순위: 하위 30%" (모니터링) |
| **P2 전환** | 행동-클릭 | CTR | text (CTA), psychology (urgency) | ✅ 처방 | "CTA를 혜택 구체화형으로 변경 → CTR +0.5%p" |

#### 랜딩 각도기 (V2 — Out of Scope)

| 여정 단계 | 성과 지표 | 처방 예시 |
|----------|----------|----------|
| 체류 | 스크롤 깊이, 체류시간 | "히어로 섹션에 핵심 혜택 명시" |
| 행동 | 장바구니, 결제시작 | "CTA 버튼 위치/색상 개선" |
| 구매 | 구매전환율 | "소재↔LP 메시지 일관성 강화" |

#### 세트 목적함수

```
reach_to_purchase_rate = 광고(CTR까지) × 랜딩(클릭→구매)
  - CTR 높은데 구매 안 됨 → LP 문제
  - CTR 낮음 → 소재 문제
  - 둘 다 낮음 → 세트로 바꿔야 함
```

#### ATTRIBUTE_AXIS_MAP 매핑 (광고 각도기 범위)
- hook 축 → 기반(P0) 영향 — 진단+처방
- visual 축 → 기반(P0) + 전환(P2) 영향 — 진단+처방
- text 축 → 전환(P2) 영향 — 진단+처방 (참여 영향은 모니터링만)
- psychology 축 → 전환(P2) 영향 — 진단+처방 (참여 영향은 모니터링만)
- quality 축 → 기반(P0) 영향 — 진단+처방
...
| **처방 범위** | 광고 각도기만 — 기반(P0) + 전환(P2=CTR) 처방. 참여(P1)는 진단만 |
...
| **참여축 UI** | 진단만 표시 (🟢🟡🔴) + "메타 참여율순위 건강도" 게이지. 처방 버튼 없음 |
| **세트 병목 표시** | reach_to_purchase_rate 기반 "CTR 높은데 구매 안 됨 → LP 점검" 안내 |
...
| 항목 | 이유 | 예상 시기 |
|------|------|----------|
| **랜딩 각도기 (V2)** | LP 처방은 LP 크롤링 완료 + lp_structure_analysis 데이터 필요. 광고 각도기 확정 후 착수 | 광고 각도기 MVP 이후 |
| **영상 소재 처방** | 영상 프레임별/씬별 처방은 복잡 (ffmpeg 씬분할 + 재생이탈곡선 필요) | 5축 배치 + DeepGaze-Gemini 파이프라인 완료 후 |
| **참여축 처방** | 상관분석 결과 참여 → 구매 직접 상관 r≈0. 액셔너블한 처방 불가. 기반/전환 개선 시 참여 따라옴 | 데이터 추가 축적 후 재검토 |
...
- **축 구조 (2026-03-25 상관분석 기반 확정)**:
  - 광고 각도기: 기반(진단+처방) + 참여(진단만) + 전환=CTR(진단+처방)
  - 랜딩 각도기: 클릭→구매 퍼널 (V2)
  - 세트 목적함수: reach_to_purchase_rate
- **범위**: 광고 각도기 처방만 (이미지 소재, on-demand, 소재 상세 페이지 탭)
- **참여축**: 메타 알고리즘 참여율순위 모니터링만. 처방 안 함 (액셔너블하지 않음)
- **선행 조건**: 5축 배치 완료 (현재 16% → 90%+ 필요)
- **예상 공수**: 5일 (STEP 1~5)
