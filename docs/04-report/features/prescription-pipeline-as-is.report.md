...
| 3 | LP → 처방 | LP 6단계 체인 전부 수동 (매핑→비전분석→상관분석→행동추론→교차매트릭스→alignment) + 처방 엔진 미연결 | LP 일관성 점수가 처방에 반영 안 됨. LP 8요소×전환율 상관 데이터도 미활용 |
...
### 2.4 LP 관련 스크립트 (scripts/archive/) — 5단계 체인

| 스크립트 | 역할 | 입력 | 출력 | 상태 |
|----------|------|------|------|------|
| populate-creative-lp-map.mjs | 소재↔LP URL 자동 매핑 (utm/fbclid 정규화) | creatives.lp_url + landing_pages.canonical_url | creative_lp_map | ❌ 수동 |
| analyze-lps-v2.mjs | Gemini 2.5 Pro LP 8카테고리 비전 분석 | lp_snapshots.screenshot_url | lp_analysis.reference_based (page_structure/pricing/social_proof/urgency/CTA/trust/psychology/UX) | ❌ 수동 |
| compute-lp-data-analysis.mjs | LP 8요소 × 실제 전환율 상관분석 | lp_analysis.reference_based + daily_ad_insights | lp_analysis.data_based (element_correlation, impact_delta%, confidence) | ❌ 수동 |
| compute-lp-behavior-inference.mjs | LP 행동 패턴 추론 | lp_analysis.data_based | 행동 패턴 | ❌ 수동 |
| compute-lp-cross-matrix.mjs | LP 교차 속성 매트릭스 | lp_analysis | cross-attribute matrix | ❌ 수동 |
| analyze-creative-lp-alignment.mjs | 소재↔LP 일관성 Gemini 평가 (message/visual/cta/offer 0-100점) | creative_lp_map + analysis_json + lp_analysis | creative_lp_map (alignment scores + issues) | ❌ 수동 |

**LP 분석 실행 순서**: populate-creative-lp-map → analyze-lps-v2 → compute-lp-data-analysis → compute-lp-behavior-inference + compute-lp-cross-matrix → analyze-creative-lp-alignment

### 2.5 처방 제안 + 변화 추적 스크립트

| 스크립트 | 역할 | 입력 | 출력 | 상태 |
|----------|------|------|------|------|
| compute-change-insights.mjs | change_log에서 변경→성과 패턴 추출 | change_log (element_diff + performance_change) | change_insights 테이블 or JSON | ❌ 수동 |
| generate-suggestion-bank.mjs | 변화 패턴에서 순위화 제안 목록 생성 | change_log + insight/tracking JSONs | suggestion-bank-output.json + lp_analysis.data_based.suggested_actions | ❌ 수동 |
| compute-suggestion-tracking.mjs | 제안 실행 추적 | suggestion_bank + change_log | tracking scores | ❌ 수동 |

### 2.6 기타

| 스크립트 | 역할 | 상태 | 비고 |
|----------|------|------|------|
| embed-all.mjs | **강의 청크** 임베딩 (Gemini text-embedding-001) | ✅ 구현 | 소재 임베딩(embed-creatives cron)과 별개 — 교육 콘텐츠용 |
...
| LP 일관성 뷰 | ❌ | UI 미구현 (v2에서 LP 제거 결정 — 믹스패널 직접 확인) |
| 피로도 경고 뷰 | ❌ | UI 미구현 (데이터도 수동) |
| 패턴 인사이트 뷰 | ❌ | UI 미구현 (데이터도 수동) |
| 관리자 페이지 | ✅ | /admin/protractor — 벌크 수집 + 상태 모니터링 |
| 역할별 접근 제어 | ✅ | admin=전체, student(연결)=자기계정, 미연결=샘플 |
...
| 4 | **LP 분석 6단계 체인 자동화 + 처방 연결** | LP 일관성이 처방에 반영 안 됨. 광고↔LP 불일치 감지 불가. 5단계 LP 분석 체인(매핑→비전분석→상관분석→행동추론→교차매트릭스) + alignment 평가 전부 수동. | populate-creative-lp-map → analyze-lps-v2 → compute-lp-data-analysis → alignment 순서로 cron 체인. 처방 엔진 STEP에 LP 데이터 추가. |
...
Meta API → collect-daily (매일 02:00)
    ...
    (06:00) (06:00)      (06:00)
         ...
         ┌──────┼─────────────────────────────────────┐
         │      │                                     │
    crawl-lps  populate-lp-map → analyze-lps-v2      │ ← ★ LP 6단계 체인
    (기존)     → lp-data-analysis → lp-alignment     │    (5개 신규 cron)
         └──────┼─────────────────────────────────────┘
                ↓
...
3. **LP 6단계 체인 단절** — 크롤링만 cron, 매핑→비전분석→상관분석→행동추론→교차매트릭스→alignment 전부 수동. 단, v2 기획에서 LP 데이터를 대시보드에서 제거하고 믹스패널 직접 확인으로 변경한 이력 있음 → LP 자동화 우선순위 재평가 필요
4. **축2 데이터 정체** — prescription_patterns 자동 갱신 안 됨 → 처방 품질 저하
5. **embed-all.mjs는 교육용** — 소재 임베딩은 embed-creatives cron이 담당. embed-all은 강의 청크 전용으로 파이프라인과 무관
