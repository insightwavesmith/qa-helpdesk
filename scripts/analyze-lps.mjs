...
async function analyzeWithVision(screenshotUrl, ctaUrl) {
  if (!GEMINI_KEY) {
    console.log("    ⚠️ GEMINI_API_KEY 없음, Vision 스킵");
    return null;
  }
  ...
  try {
    ...
    const imgRes = await fetch(visionUrl, { signal: AbortSignal.timeout(15_000) });
    if (!imgRes.ok) {
      console.log(`    ⚠️ 이미지 다운로드 실패: ${imgRes.status}`);
      return null;
    }
    ...
    if (ctaUrl) {
      try {
        const ctaRes = await fetch(ctaUrl, { signal: AbortSignal.timeout(15_000) });
        if (ctaRes.ok) {
          const ctaCt = ctaRes.headers.get("content-type") || "image/jpeg";
          const ctaMime = ctaCt.startsWith("image/") ? ctaCt.split(";")[0] : "image/jpeg";
          const ctaBuf = await ctaRes.arrayBuffer();
          const ctaBase64 = Buffer.from(ctaBuf).toString("base64");
          parts.push({ inline_data: { mime_type: ctaMime, data: ctaBase64 } });
        }
      } catch {
        // CTA 이미지 다운로드 실패는 무시
      }
    ...
    if (!res.ok) {
      ...
      console.log(`    ⚠️ Gemini Flash 실패: ${res.status} ${err.slice(0, 200)}`);
      return null;
    }
...
// ━━━ 메인 ━━━
async function main() {
  console.log("🚀 LP 분석 시작 (Gemini Flash Vision + Embedding + Consistency)");
