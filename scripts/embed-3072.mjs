#!/usr/bin/env node
/**
 * 3072차원 유사도 전용 임베딩 배치 스크립트
 * Supabase Management API + Gemini Embedding API 직접 호출
 * 사용법: SUPABASE_ACCESS_TOKEN=xxx GEMINI_API_KEY=xxx node scripts/embed-3072.mjs
 */
...
async function embedText(text) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Gemini text embedding failed: ${res.status} ${errText}`);
  }
...
async function embedImage(imageUrl) {
  ...
  if (!res.ok) {
    const errText = await res.text();
    throw new Error(`Gemini image embedding failed: ${res.status} ${errText}`);
  }
  const data = await res.json();
  return data.embedding.values;
}
...
function vecToSql(vec) {
  return `'[${vec.join(",")}]'::vector(${DIMENSIONS})`;
}
...
async function main() {
  const countResult = await supaQuery(
    "SELECT count(*) as cnt FROM ad_creative_embeddings WHERE embedding_3072 IS NULL AND is_active = true"
  );
  let remaining = countResult[0].cnt;
  console.log(`처리 대상: ${remaining}건 (${DIMENSIONS}차원)`);
  ...
  while (remaining > 0) {
    const rows = await supaQuery(`
      ...
      WHERE embedding_3072 IS NULL AND is_active = true
      ORDER BY created_at
      LIMIT ${BATCH_SIZE}
    `);
    ...
    for (const row of rows) {
      ...
      if (imgVec || txtVec) {
        const sets = [];
        if (imgVec) sets.push(`embedding_3072 = ${vecToSql(imgVec)}`);
        if (txtVec) sets.push(`text_embedding_3072 = ${vecToSql(txtVec)}`);
        sets.push(`embedded_at = now()`);

        try {
          await supaQuery(
            `UPDATE ad_creative_embeddings SET ${sets.join(", ")} WHERE id = '${escSql(row.id)}'`
          );
          totalEmbedded++;
  ...
  const final = await supaQuery(
    "SELECT count(*) as total, count(embedding_3072) as has_3072, count(text_embedding_3072) as has_txt_3072 FROM ad_creative_embeddings"
  );
