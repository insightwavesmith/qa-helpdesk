#!/usr/bin/env python3
"""
광고 소재 시선 예측 스크립트 — DeepGaze IIE 기반

Usage: python scripts/saliency-predict.py [--limit N] [--account-id xxx]
...
5. top fixation 좌표 + CTA 주목도 점수 계산
6. creative_saliency에 UPSERT

의존성: pip install -r scripts/requirements-saliency.txt
"""
...
if not SB_URL or not SB_KEY:
    print("ERROR: NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY 필요")
    sys.exit(1)
...
def sb_storage_upload(bucket: str, path: str, data: bytes, content_type: str = "image/png"):
    ...
    if res.ok:
        ...
        print(f"    Storage 업로드 실패: {res.status_code} {res.text[:200]}")
        return None
...
def get_model():
    ...
    # DeepGaze는 centerbias_template을 입력으로 받음
    # 실제로는 MIT1003 등 데이터셋의 centerbias를 사용하지만,
    # 간단히 가우시안으로 근사
...
# ━━━ CTA 주목도 점수 ━━━
def compute_cta_score(saliency: np.ndarray, cta_position: str | None) -> float | None:
...
def main():
    ...
    for i, creative in enumerate(to_analyze):
        ...
        print(f"[{i+1}/{len(to_analyze)}] {ad_id} — ", end="", flush=True)
    ...
    print(f"\n━━━ 결과 ━━━")
    print(f"총 대상: {len(creatives)}건")
    print(f"스킵(기존): {len(existing_set)}건")
