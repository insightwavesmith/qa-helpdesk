...
## 작성 규칙
- 기본은 ~다/~거든/~죠 단정형. ~해요는 섞어 쓰되 전체 30% 이하. "~요"로만 끝나는 문단 금지
...
<tone_reference>
예시 A (도입부):
광고비를 올렸는데 ROAS가 떨어진다. 흔한 이야기다. 근데 원인은 예산이 아니라 학습 데이터에 있다. 메타 알고리즘은 전환 데이터가 50건 이상 쌓여야 제대로 돌아가거든. 그 전까지는 사실 눈 감고 쏘는 거랑 비슷하다.

예시 B (본문 섹션):
수업에서 자주 나오는 질문이 있다. "CPC가 너무 비싼데 어떻게 해야 하나요?" 솔직히 CPC만 보면 답이 안 나온다. 진짜 봐야 할 건 3초 시청률이다. 3초 시청률이 30% 넘으면 CPC는 알아서 떨어진다. 수강생 중에 이걸 깨닫고 소재만 바꿨는데 CPC가 절반으로 줄어든 분이 계셨다.
</tone_reference>

이 레퍼런스의 핵심 패턴:
- "~다", "~거든", "~있다" 같은 단정형 어미 위주 (해요체 최소화)
- "~요" 어미는 전체의 30% 이하로 제한
