...
- 기본은 ~다/~거든/~죠 단정형. ~요 어미는 전체 30% 이하. "~요"로만 끝나는 문단 금지.
...
어미 다양화 규칙:
- 같은 문장 어미를 연속 3번 이상 쓰지 마라. ~다, ~거든요, ~세요, ~죠, ~한데요 등 다양하게 섞어라.
- 어미 패턴 예시: "~다" → "~거든요" → "~세요" → "~한데요" 이런 식으로 바꿔가며 써라.
- "~해요", "~되요" 반복 금지. 단정형(~다, ~거든) 위주로 쓰되 ~요를 적절히 섞어라.

문장 리듬:
- 짧은 문장(15자 이내)과 긴 문장(40자+)을 번갈아 배치.
- "결과가 나왔다. 3주간 ASC 돌린 수강생의 ROAS가 2.4배 올랐거든요." 이런 식.
- 의문문·감탄문을 적절히 삽입. "왜 이런 차이가 날까요?" "진짜 됩니다."
...
톤 레퍼런스 (이 톤을 따라해라):
- O: "결론부터 말하면 CBO 쓰세요. 이유는 세 가지다."
- O: "솔직히 이건 데이터를 봐야 한다. 지금 CTR이 얼마인지부터 확인하세요."
- O: "수업에서 자주 나오는 질문이다. CPC만 보면 답이 안 나온다. 진짜 봐야 할 건 3초 시청률이거든요."
