...
export function createNumberedCardsRow(fields: NumberedCardsFields): object[] {
  ...
  return [makeTextRow("numbered-cards", html, "8px 24px 4px")];
}
...
export function createChecklistRow(fields: ChecklistFields): object[] {
  ...
  const html = `<table width="100%" cellpadding="0" cellspacing="0" style="padding:0 0 8px;">${cards}</table>`;
  return [makeTextRow("checklist", html, "8px 24px 4px")];
}
...
/**
 * 카테고리 뱃지 row (빨간 pill 뱃지, 중앙 정렬).
 * @param label - 뱃지 텍스트 (예: "정보공유", "수강생 사례")
 */
export function createCategoryBadgeRow(label: string): object {
  return makeTextRow(
    "category-badge",
    `<table align="center" cellpadding="0" cellspacing="0"><tr><td align="center" bgcolor="#F75D5D" style="background-color:#F75D5D;color:#ffffff;font-size:12px;font-weight:700;padding:6px 18px;border-radius:20px;letter-spacing:1px;">${escapeHtml(label)}</td></tr></table>`,
    "0px 24px 12px",
  );
}
...
export function createHeroRow(title: string, subtitle: string, badgeText = "LIVE 무료 웨비나"): object {
  ...
  const subtitleHtml = subtitle
    ...
    : "";
  // C안: 10+마크 좌측정렬 + 자사몰사관학교 10 하단 맞춤
  const heroHtml = `<table cellpadding="0" cellspacing="0" style="margin-bottom:16px;"><tr>
    <td style="vertical-align:bottom;padding-right:6px;"><img src="${logoMarkUrl}" alt="10+" style="width:24px;height:auto;display:block;" /></td>
    <td style="vertical-align:bottom;color:rgba(255,255,255,0.85);font-size:12px;font-weight:700;letter-spacing:1px;line-height:1;">자사몰사관학교</td>
  </tr></table>
  ...
  columns: [{
    ...
    contents: [{
      ...
      values: {
        ...
        selectable: true, draggable: true, duplicatable: true, deletable: true, hideable: true,
        text: '<table width="100%" cellpadding="0" cellspacing="0" style="margin:24px 0;"><tr><td style="padding:24px 0;border-top:1px solid #e2e8f0;border-bottom:1px solid #e2e8f0;"><table cellpadding="0" cellspacing="0"><tr><td width="100" style="vertical-align:top;"><img src="https://bscamp.vercel.app/images/meta-partner/profile-smith.png" alt="스미스" style="width:80px;height:80px;border-radius:50%;display:block;" /></td><td style="vertical-align:top;"><p style="margin:0;font-weight:800;font-size:16px;color:#1a1a1a;">스미스 <span style="font-weight:600;font-size:13px;color:#F75D5D;">자사몰사관학교 코치</span></p><p style="margin:6px 0 0;font-size:13px;color:#64748b;line-height:160%;">Meta Business Partner로서 광고 성과를 높입니다<br>수강생 자사몰매출 450억+</p></td></tr></table><div style="margin-top:16px;padding-top:16px;border-top:1px solid #f1f5f9;"><img src="https://bscamp.vercel.app/images/meta-partner/inline-positive.png" alt="Meta Business Partners" style="height:36px;display:block;" /></div></td></tr></table>',
      },
    }],
    values: {
      backgroundColor: "", padding: "0px", border: {}, borderRadius: "0px",
