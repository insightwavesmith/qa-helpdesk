...
class StateMachine:
    ...
    def _handle_block_event(
        ...
        if event.type == "block.started":
            ...
            if block_inst.status == BlockStatus.GATE_CHECKING:
                ...
                # Find next block via links (튜플 반환)
                next_blocks, extra_link_commands = self._find_next_blocks(wf, block_id)

                # extra link commands 추가 (compete 등)
                for cc in extra_link_commands:
                    commands.append(cc)
            ...
            if on_fail == "retry" and block_inst.retry_count < max_retries:
                ...
                block_inst.status = BlockStatus.COMPLETED
                block_inst.completed_at = time.time()
                next_blocks, _extra = self._find_next_blocks(wf, block_id)
                if next_blocks:
                    for next_id in next_blocks:
    ...
    def _find_next_blocks(
        ...
        """link type과 condition을 평가하여 다음 블록 결정. (next_ids, extra_commands) 튜플 반환."""
        next_ids: list[str] = []
