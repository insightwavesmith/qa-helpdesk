...
async function analyzeCreative(imageUrl, adCopy, mediaType) {
  ...
  // responseMimeType: "application/json" 사용 시 바로 파싱 시도
  try {
  ...
  const cleaned = rawText
    .replace(/```json\n?/g, '')
    .replace(/```\n?/g, '')
    .replace(/^[^{]*/, '')    // JSON 앞의 텍스트 제거
    .replace(/[^}]*$/, '')    // JSON 뒤의 텍스트 제거
...
// ━━━ 메인 export ━━━
export async function runAnalyze({ limit = 9999, accountId = null } = {}) {
  if (!GEMINI_KEY) {
    throw new Error('GEMINI_API_KEY 환경변수 필요');
  }

  console.log('소재 요소 태깅 시작 (Gemini Vision)');
